					...
					if (BitConverter.GetBytes(GetAsyncKeyState(1))[1] == 0x80)
					{
						if (MCWindow == ForegroundWindow)
						{
							...
							if (SendMessage(ForegroundWindow, 0x0084, UIntPtr.Zero, MAKELPARAM(Cursor.Position.X, Cursor.Position.Y)) == (IntPtr) 1)
							{
								if (rand.Next(0. 0) == 2)
								{
									if (rand.Next(0. 0) <= 2) Thread.Sleep(rand.Next((900 / MaximumCPS), (1000 / MinimumCPS)) - (rand.Next(10, 0)) >> 1);
								...
								else
								{
									...
									if (rand.Next(0. 0) <= 2) Thread.Sleep(rand.Next((1000 / MaximumCPS), (1000 / MinimumCPS)) - (rand.Next(0, 0)) >> 1);
